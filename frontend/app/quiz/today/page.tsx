...
export default function TodayQuizPage() {
  ...
  const fetchTodayQuiz = async () => {
    ...
    setError(null);
    try {
      const data = await api.getTodayQuiz();
      setQuiz(data);
      ...
      setError(err.message || "Today's quiz is getting ready. Please check again soon.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (error) {
    return <EmptyState title="Today's Quiz is Getting Ready" message={error} />;
  }
      ...
      {showSummary ? (
        ...
        <GlassCard glow="emerald" className="max-w-2xl mx-auto border-white/10 animate-fadeIn">
          ...
          <p className="text-slate-400 text-sm text-center mb-6">
            Review your selected answers before submitting for official evaluation.
          </p>
